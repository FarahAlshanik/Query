...
def get_files(file_folder='', file_format='file_%d.csv'):
	...
	Usage:
		files = get_files('Boston/try/', 'file_%d.csv')
...
def build_args():
	...
	parser.add_argument("--output_folder", help="location of output files", type=str, default='Boston/lda/')
...
def get_text_from_file(file=None, stopwords=[]):
	...
	return data
'''
def calculate_LDA(input_files=[], output_folder=''):
...
#        for i in range(5):
 #               x=lda.show_topic(i, topn=5)
               # for i in x:
  #              print(x)
        #outfile = output_folder + 'lda%d.txt' % (interval + 1)
...
if __name__ == "__main__":
	...
	print(input_files)
	#calculate_LDA(input_files=input_files, output_folder=args.output_folder)
