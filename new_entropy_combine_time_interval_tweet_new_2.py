...
for k in range(len(processed_files)):
    l=0
    for i in set(processed_files[k]):
        #print(k)
        l+=(dict[entropy[k]][i]) #total freq
    le.append(l)
...
for i in range (len(entropy)):
    s=0
    for doc in processed_files[i]:
        entropy_val=entropy_fun(doc,doc,le[i],dict[entropy[i]])
        e.append([i,entropy_val])
        s+=entropy_val
    print(i)
    entropy_times.append([i,-s])




dis = open('new_entropy_combine_interval_tweet_new_2.txt', 'w')
