...
print(len(centroid_dic_combined))
#print(centroid_dic_combined[0])

#print(centroid_dic[0])
...
dd=[]
w=1
for i in centroid_dic_combined:
    if(w%4==0):
        print()
    else:
        dd.append(centroid_dic_combined[i])
    w+=1
...
for k,i in enumerate(centroid_dic):
    ...
    if(r%w==0):
        if(r!=0 and n<=3):
            sim=1-spatial.distance.cosine(centroid_dic[k],centroid_dic_combined[n])
            sim_dic.append([k,n,sim])
        elif(r!=0 and n>3):
            u+=1
            sim=1-spatial.distance.cosine(centroid_dic[k],centroid_dic_combined[n+u])
            sim_dic.append([k,n+u,sim])
        ...
        sim=1-spatial.distance.cosine(centroid_dic[k+1],dd[n])
...
dis = open('/zfs/dicelab/farah/query_exp/query-construction2/similarity_centroid_combined_sliding_3_new.txt', 'w')
for item in sim_dic:
  dis.write("%s\n" % item)
dis.close()
