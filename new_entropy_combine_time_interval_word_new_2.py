...
for k in range(len(processed_files)):
    f_w2=0
    w=[]
    for doc in processed_files[k]:
        for word in set(str(doc).lower().split()):
            if(word not in w):
                f_w2+=dict[entropy[k]][word]
            w.append(word)
    freq.append(f_w2)
...
for k in range(len(processed_files)):
    ...
    for doc in processed_files[k]:
        ...
        #print(w)
            if(w not in wor):
                entropy_val=(prob_log(w,dict[entropy[k]],freq[k]))
               # print(entropy_val)
                m+=entropy_val
            wor.append(w)
    e.append([k,m])
...
dis = open('new_entropy_combine_interval_words_new_2_new.txt', 'w')
for item in e:
  dis.write("%s\n" % item)
dis.close()
