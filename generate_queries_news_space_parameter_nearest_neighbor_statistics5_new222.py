...
def main(args):
  ...
  print("Category-2-Words that appear in the current interval but are not in the top-20 LDA words: ",len(list_future_words)-len(list_future_words2)) ## category 2 number of words in current $
 ...
 #print("Words that do not appear in the current interval but appear after but not before- number of words not in current intervals but in future ",len(f_w2)) #  category 5 number$
...
#current_interval=pd.read_csv('/zfs/dicelab/farah/balt_break_all_tweet/file_'+str(args.start_window)+'.csv',names=['a','b','c'])
  ...
  tweet_statistics.write("Category-3-Words that do not appear in the current interval but appear before but not after"+str(len(inter_before_nn))+"    ")
  tweet_statistics.write("\n")

  print('Category-3-Words that do not appear in the current interval but appear before but not after: ',str(len(inter_before_nn))) #category 3


  inter_after_befor=set(inter_before_nn)&set(inter_after_nn)
  num_words_after_before=len(inter_before_nn)+len(inter_after_nn)-len(inter_after_befor)
  print('Words that do not appear in the current interval but that have appeared both before and after ',str(num_words_after_before)) #category 4
  print(inter_before_nn)

  tweet_statistics.write("Category-4-Words that do not appear in the current interval but that have appeared both before and after: "+str(num_words_after_before)+"    ")
  tweet_statistics.write("\n")




  tweet_statistics.write("Category-5-Words that do not appear in the current interval but appear after but not before: "+str(len(inter_after_nn))+"    ") #category 5
  tweet_statistics.write("\n")
