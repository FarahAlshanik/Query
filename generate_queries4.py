...
def append_dec_keywords(decfile,ldakeywords):
  ...
  for line in dec_words:
    ...
    if(word not in dic_w[k] and word !='rt'):
      dic_words[k].append(word)
      count -= 1
   # else:
    #  count+=1
    if(count==0 and k !=len(dic_w)-1):
      #print(dic_words[k])
...
def main(args):
  ...
  #keywords from dec
  keywords = append_dec_keywords(decfile=args.dec_keyword_file,ldakeywords=lda_keywords)
  ...
  for topic_id in range(num_topics):
    ...
    for input_file in input_files:
      ...
      for line in inf:
        ...
        for word in keywords[topic_id]:
        #for word in lda_keywords[topic_id]:
