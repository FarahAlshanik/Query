...
def main(args):
  ...
  print('not in topic=',nottopicn) #either in current or not in tweet or not  it's not important category 
  print('Words that appear in the current interval in the top-20 LDA words=',intopicn) #good  category 1
  print(wnn)
  ...
  tweet_statistics.write("not in topic: "+str(nottopicn)+"    ")
  tweet_statistics.write("*******")
  tweet_statistics.write("Words that appear in the current interval in the top-20 LDA words : "+str(intopicn)+"     ") #category 1
  tweet_statistics.write("*******")
  wordss=[]
  #intweetn=0
  #notintweet=0
  ...
  print("number of words not in tweets not in stram at all: "+str(notintweet)+"    ")
  print("number of words in tweets and : "+str(intweetn-nottopicn)+"    ") #number of words from nn in stream but without intersection with lda words 
  tweet_statistics.write("number of words not in tweets: "+str(notintweet)+"    ")
  tweet_statistics.write("*******")
  tweet_statistics.write("number of words in tweets: "+str(intweetn-nottopicn)+"    ")
  tweet_statistics.write("*******")
  ...
  list_future_words = set(set_current_int)&set(list_t) #intersection between current and words in tweet  words in current 
 ...
 f_w2=set(f_w)&set(words_inter_nn_topic)
  ...
  print("Words that appear in the current interval but are not in the top-20 LDA words: ",len(list_future_words)-len(list_future_words2)) ## category 2 number of words in current intervals and are already in tweets
  print("Words that do not appear in the current interval but appear after but not before- number of words not in current intervals but in future ",len(f_w2)) #  category 5 number of words not in current intervals but in future  
  print(" words in both current and all tweets:  ",list_future_words2)
  print(" words in current interval:  ",f_w2)
  tweet_statistics.write("*******")
  tweet_statistics.write("number of words in current intervals and are already in tweets: "+str(len(list_future_words)-len(list_future_words2))+"    ") #category 2
  tweet_statistics.write("*******")
  tweet_statistics.write("*******")
  tweet_statistics.write("Words that do not appear in the current interval but appear after but not before- number of words not in current intervals but in future: "+str(len(f_w2))+"    ") #len of words in tweets - intersection between current and in give future
  ...
  for input_file in input_files:
    ...
    for line in inf:
      for word in line.lower():
        ...
        if word not in set_current_int:
