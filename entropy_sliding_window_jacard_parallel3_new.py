...
def simm(data,q,y):
    ...
    for i in range(len(data)):
        for j in range(i+1,len(data)):
            similarity.append(round(get_jaccard_sim(data[i],data[j]),2))
            #print(i,j)
...
for i in range(550,570):
    p1=Process(target=simm,args=(wind[i],q,i))
    p1.start()
    #entr.append( simm(wind[i]),q,i)
...
f1='/zfs/dicelab/farah/query_exp/query-construction2/ent_sliding_jacard_parallel1.txt'
