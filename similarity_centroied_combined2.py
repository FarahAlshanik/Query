...
sim_dic=[]
sim_dic.append([0,0,1]) #sim between (0,0)
sim=1-spatial.distance.cosine(centroid_dic[0],centroid_dic[1])
sim_dic.append([0,1,sim])# betwwen 0 and 1 because we start from sim(2,01)
...
for i in range(2,lenn):
   ...
   # if(j==lenn):
    #    break
    sim=1-spatial.distance.cosine(centroid_dic[i],centroid_dic_combined[j])
...
dis = open('/zfs/dicelab/farah/query_exp/query-construction2/similarity_centroid_combined2.txt', 'w')
for item in sim_dic:
  dis.write("%s\n" % item)
dis.close()
